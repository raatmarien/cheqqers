...
class Entangled:
    ...
    def return_all_possible_states2(self, new_all_ids=None):
        """
    ...
    def return_possible_states_adv(self, classical_squares):
        """
        ...
        unsuc_sum = sum(
            [classical_squares[str(i)].chance / 100 for i in self.unsuccessfully_takes]
        )
        for i in self.unsuccessfully_takes:
            for j in self.not_taken:
                state = []
                ...
                weights.append(
                    (
                        classical_squares[i].chance
                        / 100
                        * classical_squares[j].chance
                        / 100
                    )
                    / unsuc_sum
                )
    ...
    def remove_id(self, id: str):
        """
        Removes id from all lists
        """
        all_ids = self.all_ids
        if id in self.is_taken:
            self.is_taken.remove(id)
        if id in self.successfully_takes:
            self.successfully_takes.remove(id)
        if id in self.not_taken:
            self.not_taken.remove(id)
        if id in self.unsuccessfully_takes:
            self.unsuccessfully_takes.remove(id)
        if id in self.all_ids:
            self.all_ids.remove(id)
            if len(self.all_ids) <= 1:
                print(self.all_ids)
                print(10)
                time.sleep(10)
            return True, all_ids
...
class Checkers:
    ...
    def get_id_entangled(self, id: str):
        """
        Returns entangled object for given id
        """
    ...
    def measure_square(self, id) -> CheckersSquare:
        ...
        if not self.SIMULATE_QUANTUM:
            ...
            for i in tbr:
                self.entangled_squares.remove(i)
            return (
                self.board.peek(objects=[self.squares[str(id)]])[0][0],
                taken,
    ...
    def recursive_cal_rel_state(
        ...
        states = related_objects[curr].return_all_possible_states2(new_all_ids)
    ...
    def verify_rel_and_ent(self):
        ...
        for i in ent_ids:
            exists = False
            for j in self.related_squares:
                if i in j:
                    exists = True
                    break
            if not exists:
                print("ERROR: RELATED AND ENTANGLED DISCREPANCY")
                print("Could not find: ", type(i), i)
                print("In related squares: ", self.related_squares)
                print("Entangled ids: ", ent_ids)
                exit()
    ...
    def verify_entangle_squares(self):
        ...
        for count, i in enumerate(self.entangled_squares):
            if count == len(self.entangled_squares) - 1:
                break
            temp = deepcopy(self.entangled_squares[count + 1 :])
            for j in temp:
                for k in i:
                    if k in j:
    ...
    def verify_odds(self):
        for i in self.classical_squares.keys():
            if self.classical_squares[i].chance != 100:
                ...
                if not found:
                    raise Exception(
                        f"ERROR: ID NOT FOUND IN RELATED SQUARES. Chance is {self.classical_squares[i].chance} for id {i}"
                    )
    ...
    def can_take_piece(self, id):
        ...
        """
        blind_moves = self.calculate_blind_moves(
            id, self.player, self.classical_squares[str(id)].king
        )
    ...
    def calc_ent_states(self, move: Move_id):
        ...
        # sid = move.source_id
        # jid = jumped_id
        sup_sid = self.get_rel_squares(str(move.source_id))
        sup_jid = self.get_rel_squares(str(jumped_id))
        states = []
        weights = []
        if self.is_entangled(str(move.source_id)):
            ent_sid = self.get_id_entangled(str(move.source_id))
            if self.is_entangled(
                jumped_id
            ):  # jid is also entangled, either with the sid or different entanglement
                if ent_sid == self.get_id_entangled(
                    str(jumped_id)
                ):  # Both are entangled with the same object
                    # print("scenario 1")
                    poss_states, poss_weights = ent_sid.return_possible_states_adv(
                        self.classical_squares
                    )
                    for counter, state in enumerate(poss_states):
                        ...
                        if (
                            str(move.source_id) in cp.classical_squares.keys()
                            and str(jumped_id) in cp.classical_squares.keys()
                            and cp.classical_squares[str(move.source_id)].chance == 100
                            and cp.classical_squares[str(jumped_id)].chance == 100
                        ):
                        ...
                        cp.remove_from_rel_squares(
                            jumped_id
                        )  # should be redundant, since they are in the same list
                    ...
                    poss_sid_states, poss_sid_weights = (
                        ent_sid.return_possible_states_adv(self.classical_squares)
                    )
                    poss_jid_states, poss_jid_weights = (
                        ent_jid.return_possible_states_adv(self.classical_squares)
                    )
                    for sid_counter, state_sid in enumerate(
                        poss_sid_states
                    ):  # For all possible sid states
                        for jid_counter, state_jid in enumerate(
                            poss_jid_states
                        ):  # For all possible jid states
                            cp = self.get_copy()  # Create a copy of the game
                            for (
                                id
                            ) in (
                                state_sid
                            ):  # Update all ids to the correct value for this state
                                if id[1] == CheckersSquare.FULL:
                                    cp.classical_squares[str(id[0])].chance = 100
                                else:
                                    cp.remove_piece(id[0])
                            ...
                            if (
                                str(move.source_id) in cp.classical_squares.keys()
                                and str(jumped_id) in cp.classical_squares.keys()
                                and cp.classical_squares[str(move.source_id)].chance
                                == 100
                                and cp.classical_squares[str(jumped_id)].chance == 100
                            ):  # If both ids are there it means the source id can take the jumped id and therefore we need to remove the jumped id and move the source id
                                cp.remove_piece(str(jumped_id))

                                # Update source id
                                cp.classical_squares[str(move.target1_id)] = (
                                    cp.classical_squares[str(move.source_id)]
                                )
                                cp.classical_squares[str(move.target1_id)].id = (
                                    move.target1_id
                                )
                                cp.remove_piece(move.source_id)
                            cp.remove_from_rel_squares(move.source_id)
                            cp.remove_from_rel_squares(jumped_id)
                            cp.clean_ent_objects([ent_sid, ent_jid])
                            cp.legal_moves = cp.calculate_possible_moves(self.player)
                            cp.status = cp.result()
                            states.append(cp)
                            weights.append(
                                poss_sid_weights[sid_counter]
                                * poss_jid_weights[jid_counter]
                            )
            ...
            if len(sup_sid) > 1:  # Sid is in superposition
                ...
                poss_states, poss_weights = ent_jid.return_possible_states_adv(
                    self.classical_squares
                )
                for sid in sup_sid:
                    ...
                    for counter, state in enumerate(poss_states):
                        ...
                        for id in state:
                            if id[1] == CheckersSquare.FULL:
                                ...
                                cp.remove_piece(id[0])
                        if (
                            jumped_id in cp.classical_squares.keys()
                            and cp.classical_squares[str(jumped_id)].chance == 100
                        ):  # if in this state the jumped is there, we need to remove it
                            # Two scenario's
                ...
                for counter, state in enumerate(poss_states):
                    ...
                    if (
                        str(jumped_id) in cp.classical_squares.keys()
                        and cp.classical_squares[str(jumped_id)].chance == 100
                    ):
                        cp.remove_piece(jumped_id)
                        cp.classical_squares[str(move.target1_id)] = (
    ...
    def return_all_possible_states(self, move: Move_id):
        ...
        if self.is_entangled(str(move.source_id)) or self.is_entangled(
            str(jumped_id)
        ):  # Entanglement
            # print("ENTANGLEMENT")
            # print("DEBUG")
            ...
            len(source_ids) > 1 or len(jumped_ids) > 1
        ):  # Only superposition, no entanglement
            # Superposition
    ...
    def split_move(self, move: Move_id):
        ...
        for i in self.entangled_objects:
            i.update_entangled(
                str(move.source_id), [str(move.target1_id), str(move.target2_id)]
            )
    ...
    def remove_id_from_entanglement(self, id):
        """
        Whenever an id is removed from related squares, it also needs to be removed from entanglement
        """
        removed = False
        for i in self.entangled_objects:
            removed, ids = i.remove_id(str(id))
            if removed:  # if we have found the correct entangled object we can break
                break
        if not removed:
            return
        tbr = []
        for i in ids:
            for j in self.entangled_squares:
                if str(i) in self.entangled_squares:
                    tbr.append(j)
        for i in tbr:
